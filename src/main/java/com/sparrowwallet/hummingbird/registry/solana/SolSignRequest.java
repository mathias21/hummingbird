...
public class SolSignRequest extends RegistryItem {
    ...
    public SolSignRequest(byte[] signData, CryptoKeypath derivationPath ) {
        this.requestId = null;
    ...
    public SolSignRequest(byte[] signData, CryptoKeypath derivationPath, byte[] requestId) {
    ...
    public SolSignRequest(byte[] signData, CryptoKeypath derivationPath, byte[] requestId, byte[] address, String origin) {
        this.requestId = requestId;
        this.signData = signData;
        this.derivationPath = derivationPath;
    ...
    public static SolSignRequest fromCbor(DataItem item) {
        ...
        if(requestId != null && address != null) {
            return new SolSignRequest(signData, derivationPath, requestId, address, origin);
        } else if (requestId != null) {
            return new SolSignRequest(signData, derivationPath, requestId);
        } else if (address != null) {
            return new SolSignRequest(signData, derivationPath, address);
        } else {
            return new SolSignRequest(signData, derivationPath);
        }
    }
}
